...
if [ -n "${FORCE_HOSTNAME}" ]; then
    if [ "${FORCE_HOSTNAME}" == "auto" ]; then
        ...
        echo "------------------->>>> Hostname:"  $HOSTIPNAME " <<<<<-------------------"
        #/usr/bin/perl -p -i -e "s/bind-address = \"localhost\"/hostname = \"${HOSTIPNAME}\"/g" ${CONFIG_FILE}
        /usr/bin/perl -p -i -e "s/localhost/$HOSTIPNAME/g" ${CONFIG_FILE} 
        INFLUX_HOST=$HOSTIPNAME
    else
        #r/bin/perl -p -i -e "s/bind-address = \"localhost\"/hostname = \"${FORCE_HOSTNAME}\"/g" ${CONFIG_FILE}
...
echo "influxdb configuration file: "  "[file location: " ${CONFIG_FILE} "]"
cat ${CONFIG_FILE}
...
if [ -n "${PRE_CREATE_DB}" ]; then
    ...
    if [ -f "/var/lib/influxdb/data/.pre_db_created" ]; then
